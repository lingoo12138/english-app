#!/usr/bin/env python3
"""scripts/review-v1.17.0.py - v1.17.0 B7 P0/P1/P2 静态审查"""
import os
import re
import sys
...
V1170_CHECKS = [
    # 3 新角色
    ('src/lib/chatRoles.ts', r"id: 'doctor'", 'doctor 角色'),
    ('src/lib/chatRoles.ts', r"id: 'banker'", 'banker 角色'),
    ('src/lib/chatRoles.ts', r"id: 'police'", 'police 角色'),
    ('src/lib/chatRoles.ts', r"doctor|banker|police", '角色关键词'),

    # ChatRoleId 联合类型
    ('src/lib/chatRoles.ts', r"\| 'doctor'        // 医生", 'ChatRoleId 扩'),
    ('src/lib/chatRoles.ts', r"\| 'banker'        // 银行", 'ChatRoleId banker'),
    ('src/lib/chatRoles.ts', r"\| 'police'        // 警察", 'ChatRoleId police'),

    # 8 角色 (5 + 3)
    ('src/lib/chatRoles.ts', r"waiter.*[\s\S]{0,2000}doctor.*[\s\S]{0,2000}banker.*[\s\S]{0,2000}police", '8 角色顺序'),
]
...
def main():
    ...
    print(f"\n=== v1.17.0 修复点 ({len(V1170_CHECKS)} 项) ===")
    for filepath, pattern, label in V1170_CHECKS:
