#!/usr/bin/env python3
"""scripts/review-v1.22.0.py - v1.22.0 B12 P0/P1/P2 静态审查"""
import os
import re
import sys
...
V1220_CHECKS = [
    # taggedReviews
    ('src/lib/taggedReviews.ts', r"getReviewsByTag", '按 tag reviews'),
    ('src/lib/taggedReviews.ts', r"getReviewCountByTag", '计数'),
    ('src/lib/taggedReviews.ts', r"getAllTagsWithReviewCount", 'tag 统计'),
    ('src/lib/taggedReviews.ts', r"getReviewsByTagWithScore", '评分排序'),
    ('src/lib/taggedReviews.ts', r"sortReviewQueue", '复用 v1.11'),

    # ReviewCenter
    ('src/pages/ReviewCenter.tsx', r"from '../lib/taggedReviews'", 'import'),
    ('src/pages/ReviewCenter.tsx', r"filterTag", '过滤 state'),
    ('src/pages/ReviewCenter.tsx', r"tagStats", 'tag 统计 state'),
    ('src/pages/ReviewCenter.tsx', r"getReviewsByTagWithScore", '调 tag 加载'),
    ('src/pages/ReviewCenter.tsx', r"setFilterTag", '切 tag'),
]
...
def main():
    ...
    print(f"\n=== v1.22.0 修复点 ({len(V1220_CHECKS)} 项) ===")
    for filepath, pattern, label in V1220_CHECKS:
    ...
    # 类型守卫
    print("\n=== 类型守卫检查 ===")
    type_files = ['src/pages/WritePage.tsx', 'src/pages/AIChat.tsx', 'src/components/ErrorExplainButton.tsx', 'src/components/UsageButton.tsx', 'src/pages/ReviewCenter.tsx']
    ...
    for filepath, pattern in [
        ...
        ('src/lib/wordTags.ts', r"getWordIdsByTag"),
        ('src/lib/db.ts', r"version\(6\)"),
    ]:
        ok, msg = check_file_grep(filepath, pattern, f"保护 {os.path.basename(filepath)}")
        if ok:
            print(f"  {msg}")
