...
- 📕 **错题本** —— 自动识别反复记不住的词,一键掌握
- 🔍 **字母索引** —— 26 字母快速跳转词库(全键盘可用)
- 📷 **拍照识物** —— 拍照/上传图片,LLM 识别英文单词 + 例句(OpenRouter + Gemini 2.5 Flash 免费)
...
### 🤖 AI 集成
- 📷 **LLM 视觉识别** — OpenRouter API + Gemini 2.5 Flash(免费层)
  - 用户填 API key,数据完全本地存储
  - 可一键切换到 GPT-4o-mini / Claude 3 Haiku 等
  - 未来零迁移成本,换模型只改字符串
...
### 拍照识物
1. 去 https://openrouter.ai/keys 注册并创建免费 API Key
2. 进 设置 → 图片识别 → 填 API Key(默认模型 google/gemini-2.5-flash:free)
3. 移动端底部点"拍照"或首页点"拍照识物"快捷入口
4. 拍张照或上传图片,可选提示词(找食物 / 找动物等)
5. AI 5-10 秒后返回 1-3 个英文单词 + 中文 + 音标
...
### 拍照识物不工作
- 未配置 API Key:去 设置 → 图片识别 填 OpenRouter Key
- API Key 无效:检查是否复制完整(`sk-or-v1-...` 开头)
- 错误 429:免费层有 rate limit,稍等几分钟再试
- 错误 402:免费额度用完(隔天重置或切付费模型)
- 图片太大:限制 4MB,可用手机自带工具压缩
- 不识别某些物体:试试提示词(找食物 / 找动物等)或换图
