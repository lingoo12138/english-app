...
## 阶段3: v0.23 - v0.25 (4 周路线图前半,2026-07-23 全部提前)

### 路线图调整 (独立 verifier 反馈)
- 原始 4 周计划 -> Mavis 产品决策调整
- W1 砍中译英 Tab(已有 /translate)+ 加每日一句跟读
- W2 改错本推 W3 (W2 排程 6.5d 偏紧)
- W3 满 5d (词根 2.5d + 改错本 2.5d)
- W4 加 PWA 缓存精细化 (v0.22.2 P2-6 漏项) + 缓冲 1d -> 1.5d
- 详细: docs/ROADMAP_v23_v26.md

### v0.23 - v0.25 实际进展 (1.5d 干完 14.5d 计划)

| 版本 | 任务 | 原计划 | 实际 | 验证 |
|------|------|--------|------|------|
| v0.23.0 | W1-A 写作批改 /write | 4.5d | 0.5d | 11/11 |
| v0.23.0 | W1-C 每日一句跟读 | (随 W1-A) | subagent | (随 W1-A) |
| v0.23.1+0.23.2 | W1-B AI 对话中收藏 | 1d | 主人接管 | 6/6 |
| v0.23.3 | 静态审查 + 5 P1 修复 | 0.5-1d | 0.5d | 17/17 回归 |
| v0.23.4 | P2 收尾 (tooltip 对比度 + Ctrl+Enter) | 0.5d | 0.1d | build pass |
| v0.24.0 | W2-A AI 实时纠错 | 4.5d | 1d | 7+5=12/12 |
| v0.24.1 | 静态审查 + 1 P1 (loadChat reviews) | 0.5d | 0.1d | 13/13 回归 |
| v0.25.0 | W3-A 词根扩充 54%->64.7% (Top 2k 77.3%) | 2.5d | 0.5d | (脚本离线) |
| v0.25.0 | W3-B 改错本 /errors | 2.5d | 0.5d | 9/9 |

**v0.23 - v0.25 净增**:
- 0 P0 (新功能首次上线零 P0, 达到 v0.22 末 "v0.23 1.5d 验收" 目标)
- 7 P1 (全修) - AIChat 闭包陷阱/selTimer cleanup/tab reset/tooltip 滚动错位/跨 message tooltip/WritePage race 不存在/loadChat reviews 恢复
- 2 P2 (全修) - tooltip 对比度/Ctrl+Enter
- 18 页面 (+ WritePage + PronounceCustom + ErrorsPage)
- 5800+ 行 (+800)
- 5334 词 + 3450 词根 (64.7%) + Top 2k 77.3%
- writingErrors 统一表 (write + chat 两路)

**subagent 实战经验(累计 7 次失败)**:
- v0.13: 3 subagent 首次成功(2 写 + 1 verifier)
- v0.22: 2 subagent 都 fail
- v0.22.6: 1 subagent fail → 主人接管
- v0.22.9: 2 subagent (学习提醒 + Anki 卡片) 都 fail → 主人接管
- v0.23.0: 1 subagent (W1-B 静默失败) + 1 subagent (W1-C 成功) → 混合
- v0.23.3: 1 verifier subagent fail → 主人接管 + 静态审查
- v0.24.1: 主人接管 + 静态审查
- **模式**: 写代码/写文档/静态审查任务 subagent 经常 fail (token rate limit)
- **降级方案**: 主人接管 + 静态审查脚本 + Playwright 验证三件套
...
**v1.0 正式发布 (2026-07-23)**:
- 4 周路线图 v0.23-v0.26 完结 (5d 干完 21.5d 计划)
- v1.0 路线图 W5-W8 完结 (2.5d 干完 10d 计划)
- 总: 7.5d 干完 31.5d 计划 (4x 提前)
- 19 页面 / 13 组件 / 9 Settings 子 / 22 库 / 6800 行
- 18 单元测试 + 5 闭环集成测试
- 0 P0 / 0 P1 / 大幅 P2 收尾
- 数据迁移 + iOS PWA 完整化 + a11y 全覆盖
- 文档: README + ROADMAP + DEV_LOG + CHANGELOG + ROADMAP_v1 全部 v1.0 同步
...
## 阶段6: v1.1.1 D1 错题个性化复习 (2026-07-23) - 1d 干完 1d 计划
...
- 90 行, 4 个函数
- 词库内存缓存 (wordCache)
- extractErrorWords: 去标点/取首词/过滤非英文/去重
...
### T4 单元测试 30→42 (+12)
- extractErrorWords (7) + addErrorWordsToFavorites (2) + getErrorWordsCount/All (3)
- 42/42 全过

### T5 验证
- verify-v1.1-e2e 31/31 回归
- build + typecheck 0 错误

⚠️ Home 集成 WONTFIX: 错词入生词本后自动算入 ReviewReminderCard, 无需额外 UI

## 阶段7: v1.1.2 F1 学习卡分享 (2026-07-23) - 1d 干完 1d 计划

### 流程 (用户强调)
- 先做计划,存档 → 实施 → 改状态,存档 → 更新文档

### T1 ShareCard 组件
- 160 行, 3 套风格 (简洁/渐变/复古)
- 4 大数据 + Top 3 收藏
- 词库内存缓存

### T2-T4 Home 接入 + Modal + 风格
- Home 顶部 "📤 分享" 按钮
- ShareModal 3 步: 选风格 → 预览 → 复制
- Modal.tsx 加 children prop

### T5 单元测试 42→47 (+5)
- STYLES (1) + loadShareCardData (3) + 类型 (1)
- 47/47 全过
...
## 阶段9: v1.3.0 F2 成就墙 (2026-07-24) - 1d 干完 1d 计划
...
### T1 achievements.ts
- 160 行, 20 成就 (4 类 × 多级)
- streak/words/errors/favorites 阈值体系
- 解锁/进度/下一成就逻辑

### T2 /achievements 页面
- 200 行, 4 类网格
- 渐变进度卡 + 解锁动画 (bounce + glow)
- 进度条 + 下一成就

### T3-T5 Home + ShareCard
- Home 顶部 "🏆 成就 N/20" 入口
- ShareCard 海报加 "已解锁 N 个"
...
## 阶段10: v1.4.0 A2 词根 80% (2026-07-24) - 1d 干完 1.5d 计划
...
- 134 → 465 已知词根
- 50 root (数字/时间/度量/方向)
- 30 suffix 变体 (-tion/-ment/-able 等)
- 25 短 suffix (-ic/-al/-ent 等)
- 15 prefix (re-/un-/dis-/pre-)
- 50 高频 root
- 80 基础词
...
## 阶段11: v1.5.0 D3 单词短语用法 (2026-07-24) - 0.5d 干完 1d 计划

### 流程 (用户强调)
- 先做计划,存档 → 实施 → 改状态,存档 → 更新文档

### T1 explainUsage
- 协议: {phrases: [{phrase, meaning, example}], tip}
- Mock fallback (make/take/have + maker)
- 缓存 key: usage::${word}

### T2 WordDetail 接入
- UsageButton 复用组件 (110 行)
- 静态短语下方加 AI 推荐
...
## 累计 v0.1 - v1.5.0 (23 阶段, 8d)

- 8 周 + 7 周路线图 → 8d 完成
- 20 页面 / 19 组件 / 24 库
- 82 单元测试 + 16 闭环 (37 测试点)
- 0 关闭率
- 8 个 GitHub release tag

## 北极星 (一直未变)

> 让英语在你想用的时候就能用上 = 触发可业 + 内容能用 + 学得会
## 累计 v0.1 - v1.4.0 (22 阶段, 7d)

- 8 周 + 6 周路线图 → 7d 完成
- 20 页面 / 18 组件 / 24 库
- 78 单元测试 + 16 闭环 (37 测试点)
- 0 关闭率
- 7 个 GitHub release tag

## 北极星 (一直未变)

> 让英语在你想用的时候就能用上 = 触发可业 + 内容能用 + 学得会
## 累计 v0.1 - v1.3.0 (21 阶段, 6d)

- 8 周 + 5 周路线图 → 6d 完成
- 20 页面 (新加 /achievements) / 18 组件 / 24 库
- 72 单元测试 + 16 闭环 (37 测试点)
- 0 关闭率
- 6 个 GitHub release tag
...
- 8 周 + 3 周路线图 → 4d 完成
- 19 页面 / 16 组件 (8 + 5 通用 + Toast + Modal + ShareCard + ShareModal) / 22 库
- 47 单元测试 + 15 闭环 (31 测试点)
- 0 关闭率

## 北极星 (一直未变)

> 让英语在你想用的时候就能用上 = 触发可及 + 内容能用 + 学得会
## 累计 v0.1 - v1.1.1 (18 阶段, 3d)

- 8 周 + 2 周路线图 → 3d 完成
- 19 页面 / 14 组件 / 22 库
- 42 单元测试 + 15 闭环 (31 测试点)
- 0 关闭率
...
## v1.10.0 — W11 (2026-07-24)

### 触发
团队 W11 推荐 3 件事: 中译英 + 同义词 + 例句跟读

### 范围
- **P1 中译英 Tab** (subagent 写 384 行 + 13 测试) → 主人接管 0 修改
- **P2 同义词辨析** (subagent 0 产出) → 主人接管全做 (synonyms.ts 200 + SynonymsButton 150 + 13 测试)
- **P3 例句跟读** (subagent 改 WordDetail 一点) → 主人接管 5 测试 + 修 1 个 v1.8.0 测试
...
## v1.11.0 — W12 (2026-07-25)

### 触发
团队 W12 推荐 3 件事: FSRS + 复习智能队列 + 日报/周报

### 范围
- **P1 FSRS 算法** (subagent 写 201 行 fsrs.ts + plan.ts + 18 测试) → 主人接管修 4 测试期望 (d 转换公式 3.0-2.5)/0.17=2.94→3)
- **P2 复习智能队列** (subagent 写 reviewQueue.ts + ReviewCenter 改 + 7 测试) → 主人接管 0 修改
- **P3 日报/周报** (subagent 写 learningReport.ts 361 + ReportsPage 293 + 10 测试) → 主人接管修 5 测试 (d 转换 + 累计去重 + 鼓励文案) + useEffect [activeTab]

### 数据
- 195 → 230 单元测试 (+35)
- 0 P0 + 0 P1 + 0 P2 维持
- 23 → 24 组件 + 26 → 27 库
- 12 release tag (v1.0.0 ~ v1.11.0)
- 4 周路线图 v1.8-v1.11 全完结 (5 轮 3 producer 并行)

## v1.11.0 累计 (全项目 4 周完结)

- 12 release tag / 230+ commit / 24 组件 / 27 库
- 230 单元测试 + 16 闭环
- 102+ bug 修复
- 8500 → 9500 行代码
- 北极星 3 维度全拉满:
  - 触发可业: onboarding + 首页布局 + LLM 讲解
...
## v1.15.0 — W16 (2026-07-25)

### 触发
W16 候选 B5 自定义场景学习流 (闭环 v1.14 最后一公里)

### 范围
- 主人接管全程:
  - CustomSceneLearn.tsx 220 行 (卡片流 + TTS + 翻页 + 收藏 + 进度)
  - CustomSceneDetail 加 "📚 开始学习" 按钮
...
## v1.16.0 — W17 (2026-07-25)

### 触发
W17 候选 B6 多场景关联 (CustomSceneLearn 学完 → v1.11 复习)

### 范围
- 主人接管全程:
  - sceneReview.ts 100 行 (4 函数 + 词 ID 前缀 customScene:)
  - SM-2 初始值: easeFactor=2.5, interval=0
  - CustomSceneLearn 加 "📚 加入复习队列" 按钮
  - CustomSceneDetail 加复习状态卡片
  - 18 测试 (幂等 + 状态 + 删除)
...
### 范围
- 主人接管全程:
  - chatRoles.ts 加 3 角色 (doctor/banker/police)
  - ChatRoleId 联合类型扩
  - 改 chatRoles 测试 8 角色
  - 累积 v1.16 push 因 token 过期暂缓, 后续 push
...
## v1.18.0 — W19 (2026-07-25)

### 触发
W19 候选 B8 文件上传 (闭环 v1.14 自定义场景)

### 范围
- 主人接管全程 (token 恢复后):
  - fileUpload.ts 100 行 (5 函数 + FileReader API)
  - 支持 .txt / .md, 1MB 限制
  - CustomScenes 加 "📁 上传文件" 按钮 + 隐藏 file input
  - 自动从文件名提取标题
  - 23 测试
  - push 累积 (v1.16.0 + v1.17.0 + v1.18.0 一起)

### 数据
- 396 → 419 单元测试 (+23)
- 32 → 33 库
- 0 P0 + 0 P1 + 0 P2 维持
- GitHub token 替换, push 累积 3 个 tag

## v1.19.0 — W20 (2026-07-25)

### 触发
W20 候选 B9 学习日历 (复用 v1.11 learningReport)

### 范围
- 主人接管全程:
  - learningCalendar.ts 150 行 (9 函数 + 5 等级热力图)
  - CalendarPage 200 行 (7×N 网格 + 月份切换 + 统计卡片)
  - 路由 /calendar (lazy load)
  - Home 加 "📅 学习日历" 入口
  - 21 测试
...
- 33 → 34 库 + 24 → 25 页面
- 0 P0 + 0 P1 + 0 P2 维持
- 20 release tag (v1.0.0 ~ v1.19.0)

## v1.19.0 累计 (全项目 5+ 周完结)

- **20 release tag** / 290+ commit / **25 页面** / **26 组件** / **34 库** / 11500+ 行
- **440 单元测试** + 16 闭环
- 110+ bug 修复
- 8 角色对话 (面试/咖啡/前台/导游/服务员/医生/银行/警察)
- 自定义场景完整闭环 (粘贴/上传 → 提取 → 学习 → 复习)
- 错误恢复 + LLM 日限 + 拍照场景
