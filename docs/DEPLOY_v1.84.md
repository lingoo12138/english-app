...
# v1.85-v1.93 后续累计 (2026-08-01)

## 9 版本累计交付 (W78-W87)

| 版本 | 重点 |
|------|------|
| v1.85 | 听写 + 单词卡 + 跟读 3 大新功能 + 8 闭环 |
...
| **v1.93** | 错题复习模式 (Flashcard, **2 verifier 抗审查**) |

## 数据累计 (v1.93.0)

- 93 release tag / 17 周 / 22 次大 review
- **939 单元测试 / 68 文件** (v1.85 805 → 939, +134)
- **5,423 词 / 100% 词根** (1-9 字符) / 5,129 短语 (94.6%)
- **20 篇课文** / **244 同义词**
...
- main: 4027bf9 v1.93.0
- gh-pages: 09242d4 v1.93.0 ✓
- 预览: https://lingoo12138.github.io/english-app/ (HTTP/2 200)

## Verifier 抗审查 (W87+ 关键价值)

**verifier 抗审查 = 2-3 独立 sub-agent 并行, 找对抗性 bug.**

W87 实际效果:
- Verifier A (算法/状态): 3 P1
- Verifier B (业务/UX): 4 P0
- 主人修 v1 全修: 字符权重 + multiset + 队列 + 偷看 0 + useEffect focus + 4 入口空态 + 加载错误态
- 主人单独 review 漏 90% — verifier 抗审查成为新标准流程

## 真机测试 5 步 (v1.93.0 验收)

主流程已收, 真机测试 5 步覆盖:
1. **首页 + dark mode** (2 min) — 每日一句 + nav 切换
2. **词库 + 详情** (3 min) — 触类旁通跳转
3. **错题复习** (3 min) — 队列模型 + 偷看 0 + 答对移出
4. **听写 + 拼写 + 跟读** (4 min) — 字符评分 + LCS diff
5. **释义收藏 + 错题合并** (3 min) — 5 tab filter
